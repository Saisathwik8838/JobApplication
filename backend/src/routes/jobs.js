...
export function jobsRouter(dependencies) {
  ...
  router.post('/:id/analyze', validate(idParams, 'params'), async (request, response, next) => { try { const job = await getJobOrThrow(prisma, request.params.id); const result = await analyzeJob({ prisma, provider, profile, profileVersion, resumeText: await dependencies.readMasterResume(), job, logger }); response.json(result); } catch (error) { next(error); } });
  router.post('/:id/prepare', validate(idParams, 'params'), validate(prepareSchema), async (request, response, next) => { try {
    const job = await getJobOrThrow(prisma, request.params.id); const lastMatch = job.matches[0]; if (!lastMatch) { const error = new Error('Analyze the job before preparation.'); error.status = 409; throw error; }
    const match = lastMatch.result; if (match.matchScore < config.MATCH_THRESHOLD || match.recommendation === 'reject') { const error = new Error('Job does not meet the configured preparation threshold.'); error.status = 422; throw error; }
    const generated = await generateTailoredResume({ provider, profile, job, masterResumePath });
    const candidate = await prisma.candidate.upsert({ where: { profileVersion }, update: { profile }, create: { profileVersion, profile } });
    const sourceResume = await prisma.resume.create({ data: { candidateId: candidate.id, sourcePath: masterResumePath, contentHash: generated.sourceHash } });
    const version = await prisma.resumeVersion.create({ data: { resumeId: sourceResume.id, jobId: job.id, content: generated.text, claims: generated.sourceReferences, promptVersion: generated._meta.promptVersion } });
    const application = await prisma.application.upsert({ where: { jobId: job.id }, update: { resumeVersionId: version.id }, create: { jobId: job.id, resumeVersionId: version.id, status: 'MATCHED' } });
    const answers = request.body.questions.map((question) => resolveAnswer(question, profile));
    if (answers.length) { await prisma.applicationAnswer.deleteMany({ where: { applicationId: application.id } }); await prisma.applicationAnswer.createMany({ data: answers.map((answer) => ({ applicationId: application.id, ...answer, sourceRefs: answer.sourceRefs })) }); }
