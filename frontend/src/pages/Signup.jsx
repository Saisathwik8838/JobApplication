...
export function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await post('/api/auth/signup', { email, password, name });
  ...
  return (
    <div style={{ maxWidth: '420px', margin: '3.5rem auto', padding: '2rem', background: '#fff', borderRadius: '10px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', border: '1px solid #e2e8f0' }}>
      <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
        <h2 style={{ margin: '0 0 0.5rem 0', color: '#0f172a', fontSize: '1.6rem' }}>Create an Account</h2>
        <p style={{ margin: 0, color: '#64748b', fontSize: '0.9rem' }}>Start automating truthful job applications</p>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <div>
          <label htmlFor="name" style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: '#334155', marginBottom: '0.35rem' }}>
            Full Name
          </label>
          <input
            id="name"
